...
// Simple SIP client for testing calls
...
async function testCall() {
  console.log('=== Call Test for Refactored Server ===');
  
  const client1 = new CallTestClient('100');
  const client2 = new CallTestClient('101');
  ...
  console.log('Making call from 100 to 101...');
  client1.makeCall('101');
  
  // Wait for call processing
  await new Promise(resolve => setTimeout(resolve, 5000));
  
  if (client2.receivedInvite) {
    console.log('✓ Call was processed successfully');
    console.log('✓ Refactored SIP server handles calls correctly');
  } else {
    console.log('✗ Call processing failed');
  }
